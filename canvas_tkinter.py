    ...
    def jouer(self):
        ...
        if self.board.is_game_over():
            ...
            if not self.game_saved:
                save_game(
                    self.Joueur_Blanc.name,
                    self.Joueur_Noir.name,
                    result,
                    self.history_white,
                    self.history_black
                )
        ...
        if self.board.turn == WHITE:
            move = self.Joueur_Blanc.coup()
            self.board.push_uci(move)
            self.update_history_white(move)
        else:
            move = self.Joueur_Noir.coup()
            self.board.push_uci(move)
            self.update_history_black(move)
